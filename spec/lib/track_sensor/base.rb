...
shared_examples 'track sensors' do
  ...
  let(:device_data) { '' }
  let(:subscriber) { Subscriber.new('race results').wrapped_object }
  ...
  describe '#race_results' do
    context 'with results queued' do
      ...
      it 'returns the results in track order' do
        expect(subscriber).to receive(:message).with 'race results', [
          {time: 3.1, track: 1},
          {time: 3.2, track: 2},
          {time: 3.3, track: 3},
          {time: 3.4, track: 4},
    ...
    context 'when there are no results' do
      it 'returns nil' do
        expect(subscriber).not_to receive(:message)
        track_sensor.async.run
      end
    end
  end
